---
title: References
status: draft
updated: 2026-06-05
---

References are the factual and contextual part of the playbook. Use them when you need to look something up, understand how the NHS App currently works, or review supporting analysis behind the active guidance.

## Strategy and planning

- Vision and strategy
- [Roadmaps](roadmaps.md)
- Product board
- Quarterly planning
- Delivery plans

## Scope and service context

- [Current NHS App services](app-services.md)
- [NHS App user needs](user-needs.md)
- [App and operating system versions](app-os-versions.md)
- Service management

## Operations and communities of practice

- [UCD ops](ucd-ops.md)
- [Communities of practice for user-centred design](ucd-communities-of-practice.md)

## Platform model and technical context

None yet.
