# NHS App: Tenant Technical Playbook
...
For most of its life, the NHS App has been built and maintained by a single, tightly-controlled engineering team. That model has served the service well: it produced a consistent, safe, and reliable product. But it has a ceiling. The number of teams across NHS England who want to deliver functionality through the NHS App is growing — and a single builder team cannot scale to meet that demand without sacrificing the quality and reliability that 13 million people depend on.

This Playbook describes how we evolve from that model.
...
| Clinical assurance coordination | ✅ Owns process | Provides evidence, attends, and signs off |
...
| Tenant runbooks and knowledge base entries | Gate at Definition of Done | ✅ Produces |
...
2. **Run & Maintain (R&M) owns coordination.** R&M manages communications to users, coordinates across teams if the incident has cross-cutting impact, and controls the communications channels (`#alerts-high`, `#highsev-incident-updates`, in-app messaging via N&M).
3. **The NHS App team (R&M) owns the kill switch.** R&M can deactivate any tenant feature at any time, without prior agreement from the tenant team, if it is causing harm to the overall service or to patients.

The NHS App team is not responsible for diagnosing or fixing tenant features. If a tenant feature is causing a P1 incident and the tenant team is unresponsive, R&M will deactivate the feature. The tenant will fix the underlying fault before reactivation is considered.
...
A feature is not complete when code is merged. It is complete when **all** of the following are true:
...
The goal of the technical guardrails is to allow tenant teams to ship changes to 13 million monthly active users **without requiring human review from the NHS App Core team for every change**. That is only possible if tenant code is trustworthy by default — and trustworthiness is built through automation, not through review gates.

### 2.1 What makes a change "safe"

A change is safe to ship without NHS App Core team intervention when all of the following are true:
...
**Why this matters:** Human review by the NHS App Core team at this scale is a bottleneck that will break the model. The answer to "how do we know this is safe?" must be automation, not gatekeeping. This does not mean quality is lower — it means quality is enforced differently, and earlier.
...
- PRs must not remain open and unactioned for more than [TBD] working days
...
The web application is a Vue-based micro-frontend architecture. Tenant teams deliver web features as micro-frontends.
...
- Tenants must track **`-latest`** on all curated internal dependencies (design system, shared utilities). Pinning to a fixed version is not permitted without a time-bound exception granted by the NHS App Core team
...
- The BFF handles authentication, data transformation, and NHS App-specific protocol requirements
...
- All APIs follow **Semantic Versioning (SemVer)**. Breaking changes always increment the major version and are preceded by a deprecation period for the prior major version before support is withdrawn.
- All shared web components (design system, shared utilities) follow SemVer.
- Native SDK components follow SemVer, with the additional constraint that breaking changes must account for App Store review cycle timelines when calculating the minimum deprecation window.
...
- Tenant web teams must deploy on every commit to their main branch
...
The NHS App team maintains two explicit ownership maps: one for web sections, one for native sections. Every discrete section of the user interface in both channels has a named owning team.

These maps are:
- Accessible to all teams in the platform
- Updated within [TBD] working days of any ownership change
- The authoritative source of record for "who do I contact about X?"
...
| Activate a platform-wide change | The NHS App team (R&M) |
| Deactivate a tenant feature | The tenant team **or** the NHS App team (R&M) |
| Emergency deactivation of any feature on any channel | R&M — always, without prior notice or consultation |

**The NHS App team (R&M) always retains the unconditional right to deactivate any feature, on any channel, at any time.** This is not a statement of distrust toward tenant teams — it is a requirement of operating a platform used by 13 million people, some of whom may be in clinical settings when a failure occurs.

R&M are the people who are called when something is broken. They are the most motivated party to ensure that activations are safe, and they are the ones who bear the operational consequences of a bad release. Their unconditional deactivation authority reflects that accountability.

### 5.3 Web channel — rollout mechanics

- Tenant teams define the rollout cohort: ODS code, percentage of the total population, or any user data attribute available through the theming system
- **R&M executes the flag activation.** The tenant team defines the cohort and requests activation; R&M presses the button. Tenants do not have direct access to production feature flag controls.
- Service Journey Rules (SJRs) govern content entitlement (which users see which content). Feature flags govern feature activation (whether a feature is on or off). These are separate mechanisms and must not be conflated.
- All SJRs must be visible to all tenants through a platform-provided dashboard — no tenant should be surprised by another tenant's entitlement configuration affecting their feature's behaviour
...
### 6.1 Phase 0 — Internal dogfooding

**Before any external team follows this Playbook, the NHS App team will run a full internal pilot using the exact process described here.** An NHS App internal team will act as the first tenant — going through every step from initial engagement through to a feature live in production — using only the tooling, documentation, and support structures this Playbook describes.

The internal pilot is not a dress rehearsal. It is a quality gate. The Playbook and its supporting infrastructure are not ready for external teams until they have been proven to work end-to-end internally.

Required outputs of Phase 0:

- An updated version of this Playbook reflecting every gap, ambiguity, or friction point found during the pilot
...
- A clear, explicit go/no-go decision for the first external tenant cohort

**Why this matters:** Asking external teams to follow a process the NHS App team has never followed themselves is disrespectful of their time and is very likely to fail publicly. Dogfooding consistently surfaces issues that design reviews do not. The first external tenant should arrive at an onboarding experience that has already been improved by at least one complete pilot run.
...
The onboarding process has two tracks — **Web** and **Native**. Choose the track that matches your delivery channel. If you are delivering across both web and native, you go through both tracks in parallel or in sequence as agreed with the NHS App platform team.
...
3. **Technical readiness assessment:** complete the readiness checklist to confirm your team has the prerequisites: relevant language and framework skills, tool access, IG clearances, and a nominated on-call contact registered with R&M
...
- [ ] Runbook drafted and reviewed by R&M before first production release
- [ ] SJR and entitlement configuration reviewed with R&M
- [ ] SBOM entry created and registered
...
- [ ] Runbook drafted and reviewed by R&M before first App Store submission
- [ ] SBOM entry created and registered
- [ ] Supported app version range agreed with NHS App native platform engineer

#### Going live (both tracks)

1. **Staging validation:** full integration test on the shared staging environment, with a go/no-go sign-off from both the tenant team and the NHS App platform team
2. **R&M briefing:** brief R&M on your feature — its kill-switch location and operation, escalation contacts, known risks, and any platform dependencies — before production release. Do not assume R&M will find this information in a runbook under incident conditions.
3. **Phased activation:** start with a restricted cohort (see Parts 5.3 and 5.4). Do not release to 100% of users on day one under any circumstances.
4. **Post-release monitoring period:** a mandatory monitoring window during which the tenant team is in active monitoring posture. The duration of this window is agreed at onboarding based on feature scope and risk. The feature is not considered stable until the monitoring window closes with no significant issues.
...
Each tenant team is responsible for the running costs of their own backend services within the NHS Azure tenant — compute, storage, and any data transfer attributable to their services.
...
| **Tenant teams managing their own shared infrastructure** | Leads to duplication, inconsistency, and security fragmentation across the platform. |
